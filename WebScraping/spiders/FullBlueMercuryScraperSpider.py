...
class BlueMercurySpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for page in range(1, 100):
            ...
            if _response_:
                product = WebscrapingItem()
                # prod_list = {}
                for prod_ in range(len(_response_.json())):
                    product_info = _response_.json()[prod_]

                    product['Website'] = 'Blue Mercury'
                    product['Product_Category'] = dir_name
                    product['Product_ID'] = product_id = product_info['id']
                    product['Product_Name'] = product_name = product_info['title']
                    product['Product_URL'] = product_url = f'https://bluemercury.com{product_info["url"]}'
                    product['Product_Price'] = product_info['price']
                    ...
                    reviews_raw_ = requests.get('https://api.bazaarvoice.com/data/reviews.json', params=params_raw)
                    total_results = reviews_raw_.json()['TotalResults']
                    average_rating = ''
                    try:
                        average_rating = \
                        reviews_raw_.json()['Includes']['Products'][f'{product_id}']['ReviewStatistics'][
                            'AverageOverallRating']
                    except:
                        pass

                    product['Product_Rating'] = average_rating
